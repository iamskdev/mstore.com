...
export async function loadRatingModal() {
  try {
    const response = await fetch('./source/modals/rating/rating-modal.html');
    if (!response.ok) {
...
export function initRatingModal(modal) {
  ...
  const experienceOptions = {
      negative: [
          { tag: 'slow', icon: 'fas fa-clock', text: 'Slow' },
  ...
  submitBtn.addEventListener('click', async function() {
      ...
      try {
        ...
        // --- DEBUG LOGS ---
        console.log(`[Rating Modal] Auth State: isLoggedIn=${isLoggedIn}, role=${role}, userId=${userId}`);
        
        const merchantId = (role === 'merchant') ? localStorage.getItem('currentMerchantId') : null;
        console.log(`[Rating Modal] Retrieved merchantId from localStorage: ${merchantId}`);

        const appVersion = getAppConfig()?.app?.version || 'unknown';
        const comment = modal.querySelector('#commentTextarea').value.trim();

        const newRatingDoc = {
          meta: {
            ratingId: ratingId,
            // FIX: The 'type' and 'version' fields were missing from the meta object.
            type: "rating",
            version: 1.0,
            flags: {
              reviewed: false,
              responded: false,
              archived: false,
              isHighImpact: currentRating <= 2,
              guest: !isLoggedIn // NEW: Add guest flag for security rules
            },
            links: {
              // NEW: The userId of the person taking the action, regardless of role.
              userId: userId,
              orderId: null, // Example: Can be populated if rating is for an order
              itemId: null,
              // NEW: The merchantId of the submitter, if the submitter is a merchant.
              // This provides full context in the links object.
              merchantId: merchantId
            }
          },
          submitter: {
            // The primary ID of the entity submitting the rating.
          ...
          details: {
            value: currentRating,
            comment: comment,
            tags: selectedExperience ? [selectedExperience] : []
          },
          context: {
            source: "manual_trigger", // e.g., from drawer or account page
            triggerEvent: "user_initiated_rating",
          ...
          lifecycle: {
            status: "pending",
            milestones: { reviewedAt: null, respondedAt: null, archivedAt: null },
            history: []
          }
        };

        await ratingRef.set(newRatingDoc);

        // Success UI
        this.style.background = 'linear-gradient(135deg, #10b981, #059669)';
        this.innerHTML = '<i class="fas fa-check"></i> Submitted';
        console.log(`✅ Rating submitted successfully with ID: ${ratingId}`); // FIX: Log only the ID
        showToast('success', 'Thank you for your valuable feedback! 🌟');
        createConfetti();
        // FIX: Close the modal immediately after showing success UI, removing the unnecessary delay.
        setTimeout(closeModal, 800); // A short delay to let the user see the "Submitted" state.
      } catch (error) {
        console.error("❌ Error submitting rating:", error);
        showToast('error', 'Could not submit rating. Please try again.');
        this.disabled = false;
        this.innerHTML = 'Submit'; // Reset button text on error
      } finally {
        isSubmitting = false;
        // Don't re-enable the button on success, as the modal will close.
      }
  });
