...
- **Comprehensive Caching:** All common scripts, styles, and pages cached for offline use
- **Dedicated Offline Module:** Centralized offline management in `source/common/scripts/offline.js`
...
-   **प्रोग्रेसिव वेब ऐप (PWA) with Auto-Healing:**
    ...
    -   **डेडिकेटेड ऑफ़लाइन मॉड्यूल:** `source/common/scripts/offline.js` में सभी ऑफ़लाइन कार्यक्षमता केंद्रीकृत की गई है।
...
-   **एकीकृत 'dashboard' स्क्रीन (Unified 'dashboard' Screen):**
...
-   **URL Migration & Resilience System:** App automatically detects and adapts to GitHub Pages, custom domains, and localhost. Supports seamless repository renames and domain migrations with user notifications.
    ...
    -   **व्यापक 'Add Item' इंटरफ़ेस:** उत्पाद/सेवा टॉगल, गतिशील रूप से भरे जाने वाले फॉर्म, मीडिया अपलोड और फोटो गैलरी सपोर्ट के साथ।
    -   **QR बारकोड स्कैनिंग:** व्यापारियों को उत्पाद बारकोड स्कैन करके तुरंत आइटम डेटा भरने की अनुमति देता है, जिससे इन्वेंट्री प्रबंधन में गति और सटीकता बढ़ती है।
    -   **उन्नत स्टॉक प्रबंधन:** `inventory.isAvailable` फ़ील्ड के साथ स्टॉक उपलब्धता तर्क को परिष्कृत किया गया, जो दृश्यता (`isActive`) से अलग है।
    -   **फॉर्म सत्यापन और त्रुटि हैंडलिंग:** व्यापारी स्वामित्व सत्यापन और व्यापक फॉर्म सत्यापन के साथ मजबूत डेटा अखंडता।
...
-   **उन्नत मीडिया एडिटर (Advanced Media Editor):**
    ...
    -   **File Picker Best Practices (डिवेलपर नोट):**
        -   कभी भी `input[type="file"]` को `display:none` या `disabled` करके पूरी तरह से DOM से निकालने की कोशिश न करें — कई ब्राउज़र्स और PWA contexts में यह प्रोग्रामैटिक `click()` या `showPicker()` को ब्लॉक कर सकता है.
        -   यदि आप इनपुट छुपाना चाहते हैं तो उसे विज़ुअली हाइड (opacity:0, position:absolute over wrapper) रखें ताकि ब्राउज़र इसे एक वैध user-interactive control माने.
        -   हमेशा fallback के रूप में `label[for="inputId"]` का उपयोग रखें — यह एक सच्चा user gesture है और कुछ privacy blockers के बावजूद काम कर सकता है।
        -   परीक्षण के लिए Edge/Chrome/Firefox में ट्रैकिंग प्रिवेन्शन सेटिंग्स को चेक करें; privacy settings या extensions फाइल पिकर के व्यवहार को प्रभावित कर सकते हैं।
        -   यदि किसी व्यू/मॉड्यूल में file inputs न काम कर रहे हों, जांचें कि initialization के दौरान उन्हें `disabled` नहीं किया जा रहा है और स्क्रिप्ट सही तरीके से attach हो रही है।
    ...
    -   **मीडिया एडिटर एन्हांसमेंट्स:**
        -   **सुरक्षित क्षेत्र ओवरले:** इमेज क्रॉपिंग के लिए 16:9 सुरक्षित क्षेत्र ओवरले जोड़ा गया, जिसमें फिट, रीसेट और दिशानिर्देश टॉगल नियंत्रण शामिल हैं।
        -   **फाइल पिकर बेस्ट प्रैक्टिसेज:** `input[type="file"]` तत्वों को संभालने के लिए दिशानिर्देश अपडेट किए गए ताकि ब्राउज़र और PWA संदर्भों में विश्वसनीय कार्यक्षमता सुनिश्चित हो सके।
    -   **मर्चेंट प्रोफ़ाइल इमेज अपलोड और डिस्प्ले:**
        -   **लोगो और कवर इमेज अपलोड:** व्यापारियों को सीधे अपनी प्रोफ़ाइल से लोगो और कवर इमेज अपलोड और संपादित करने की अनुमति देता है।
        -   **स्केलेटन लोडिंग:** इमेज लोडिंग के दौरान एक सहज उपयोगकर्ता अनुभव के लिए लोगो और कवर इमेज के लिए स्केलेटन लोडिंग स्टेट्स लागू किए गए।
        -   **Cloudinary एकीकरण:** चैट, वार्तालाप, मर्चेंट प्रोफ़ाइल और अपडेट मॉड्यूल में Cloudinary URL बिल्डिंग को एकीकृत किया गया ताकि सभी इमेज कुशलता से परोसी जा सकें।
        -   **प्रोफ़ाइल एडिट नेविगेशन फिक्स:** प्रोफ़ाइल एडिट पेज पर नेविगेट करने से पहले `merchantId` को सही ढंग से कैश किया जाता है।
...
-   **केंद्रीकृत प्रमाणीकरण तर्क:** उपयोगकर्ता और खाता दस्तावेज़ बनाने का तर्क अब `AuthService` में केंद्रीकृत है, जो `user-collection.js` और `account-collection.js` जैसी पुरानी फ़ाइलों को हटा देता है। यह एक अधिक सुव्यवस्थित और मजबूत साइन-अप प्रक्रिया सुनिश्चित करता है।
    ...
    -   **व्यापारी आइटम CRUD एकीकरण (Merchant Item CRUD Integration):** व्यापारियों के लिए Firebase-आधारित पूर्ण आइटम प्रबंधन कार्यक्षमता लागू की गई है। इसमें गतिशील फॉर्म पॉपुलेशन, मीडिया अपलोड हैंडलिंग, व्यापक सत्यापन और Firestore सुरक्षा नियमों के माध्यम से स्वामित्व सत्यापन शामिल है।

---

## 🚦 उन्नत रूटिंग सिस्टम (Advanced Routing System)

mStore में एक शक्तिशाली, भूमिका-आधारित रूटिंग सिस्टम लागू किया गया है जो सुरक्षा, पैरामीटर सत्यापन, उन्नत URL नेविगेशन, और डायनामिक टैब रूटिंग का समर्थन करता है।

### मुख्य घटक (Core Components)

1.  **`route-manager.js`**: मुख्य रूटिंग इंजन जो URL हैंडलिंग, हिस्ट्री मैनेजमेंट, व्यू स्विचिंग, और इवेंट-ड्रिवेन अपडेट्स को संभालता है।
2.  **`route-validator.js`**: व्यापक सुरक्षा और सत्यापन इंजन जो भूमिका-आधारित एक्सेस कंट्रोल, पैरामीटर सत्यापन, रेट लिमिटिंग, और SQL इंजेक्शन/ XSS सुरक्षा प्रदान करता है।
3.  **`route-url-handler.js`**: URL पार्सिंग और निर्माण के लिए यूटिलिटीज, जिसमें पैरामीटराइज़्ड रूट्स (`item-details/:id`) और SEO-फ्रेंडली URL निर्माण का समर्थन।
4.  **`routing/index.js`**: सभी रूटिंग मॉड्यूल्स का मुख्य एंट्री पॉइंट और विरासत सिस्टम के साथ एकीकरण।

### प्रमुख विशेषताएं (Key Features)

-   **भूमिका-आधारित रूटिंग:** अलग-अलग उपयोगकर्ता भूमिकाओं (guest, consumer, merchant, admin) के लिए विशिष्ट रूट्स।
-   **URL-आधारित टैब नेविगेशन:** व्यापारियों के लिए उन्नत URL रूटिंग जैसे `#/merchant/dashboard/analytics`, `#/merchant/dashboard/inventory`।
-   **पैरामीटर सत्यापन:** सभी रूट पैरामीटर्स को सैनिटाइज़ और वैलिडेट किया जाता है, जिसमें रेगुलर एक्सप्रेशन-आधारित नियम।
-   **सुरक्षा लेयर:** रेट लिमिटिंग, SQL/XSS प्रोटेक्शन, और बिजनेस रूल वैलिडेशन।
-   **ऑफ़लाइन सपोर्ट:** सर्विस वर्कर में सभी रूटिंग मॉड्यूल्स को कैश किया गया है।

### रूट मैपिंग उदाहरण (Route Mapping Examples)

```javascript
// Merchant routes with hierarchical dashboard routing
merchant: [
  'home',
  'dashboard',           // Main dashboard (redirects to analytics)
...
// #/merchant/dashboard (redirects to analytics)
```
...
```
mstore/
├── docs/                # विस्तृत दस्तावेज़ीकरण (जैसे workflow.md, commit-guide.md)
├── localstore/
...
│   │   ├── merchant/    # व्यापारी-विशिष्ट मॉड्यूल
│   │   │   ├── analytics/ # एनालिटिक्स सब-मॉड्यूल
│   │   │   └── ...       # अन्य व्यापारी सब-मॉड्यूल
│   │   └── ...
│   ├── routing/         # उन्नत रूटिंग सिस्टम
│   │   ├── route-manager.js     # मुख्य रूटिंग इंजन
│   │   ├── route-url-handler.js # URL हैंडलिंग और SEO
│   │   ├── route-validator.js   # सुरक्षा और सत्यापन
│   │   ├── routes.js           # व्यू कॉन्फ़िगरेशन और रूट मैपिंग
│   │   └── index.js            # रूटिंग API और एकीकरण
│   ├── settings/        # ऐप कॉन्फ़िगरेशन और सेटिंग्स
│   ├── templates/       # पुन: प्रयोज्य HTML टेम्प्लेट्स
...
### Cloudinary मीडिया संरचना (Cloudinary Media Structure)

Cloudinary में मीडिया को व्यवस्थित करने के लिए एक मानकीकृत और केंद्रीकृत दृष्टिकोण का उपयोग किया जाता है। यह सुनिश्चित करता है कि सभी asset paths सुसंगत, भविष्य के लिए तैयार और आसानी से प्रबंधित हों।

#### कार्यनीति (The Strategy)

हमारी रणनीति दो प्रकार की asset handling पर आधारित है:

1.  **Fixed Public IDs (एकल, बदलने योग्य Assets के लिए):**
    *   **क्या:** User avatars, merchant logos, और cover photos जैसी assets के लिए, हम एक निश्चित, अनुमानित `public_id` का उपयोग करते हैं (जैसे `.../logos/logo`)।
    *   **क्यों:** यह हमें आसानी से इन assets को update/overwrite करने की अनुमति देता है। हम extension-less IDs (बिना `.png` या `.jpg` के) का उपयोग करते हैं ताकि भविष्य में on-the-fly format transformation (जैसे `.webp` या `.avif` में बदलना) के लिए लचीलापन बना रहे।

2.  **Folder-based Uploads (Asset Collections के लिए):**
    *   **क्या:** Item gallery images या post images जैसी assets के लिए, जहाँ एक entity (जैसे item) के पास कई images हो सकती हैं, हम एक विशिष्ट folder path (जैसे `.../items/ITEM_123/`) परिभाषित करते हैं।
    *   **क्यों:** हम Cloudinary को उस फ़ोल्डर के अंदर एक unique, random filename generate करने देते हैं। यह सुनिश्चित करता है कि एक ही आइटम के लिए कई छवियां बिना एक-दूसरे पर लिखने के मौजूद हो सकती हैं।

#### कार्यान्वयन (The Implementation)

Asset paths बनाने का सारा logic `source/api/cloudinary.js` में `getCloudinaryPath(assetType, ids)` फ़ंक्शन के अंदर केंद्रीकृत है।

**यह फ़ंक्शन सभी asset paths के लिए "single source of truth" है।** Codebase में कहीं भी manual path strings बनाने के बजाय, डेवलपर्स को हमेशा इस helper function का उपयोग करना चाहिए।

**उदाहरण:**
```javascript
import { getCloudinaryPath } from './source/api/cloudinary.js';

// एक merchant logo के लिए fixed public_id प्राप्त करना
const logoPublicId = getCloudinaryPath('MERCHANT_LOGO', { merchantId: 'MRC-123' });
// Returns: 'mstore/assets/merchants/MRC-123/logos/logo'

// एक item की image gallery के लिए folder path प्राप्त करना
const itemGalleryFolder = getCloudinaryPath('ITEM_IMAGES_FOLDER', { merchantId: 'MRC-123', itemId: 'ITM-456' });
// Returns: 'mstore/assets/merchants/MRC-123/items/ITM-456'
```

#### अद्यतन संरचना (Updated Structure)

यह संरचना `getCloudinaryPath` फ़ंक्शन द्वारा उत्पन्न वास्तविक paths को दर्शाती है:
...
└── assets/
    ...
    │   └── USER_ID/
    │       └── avatars/
    │           └── avatar  # (Fixed Public ID) User's main profile picture
    │
    ├── merchants/
    │   └── MERCHANT_ID/
    │       ├── logos/
    │       │   └── logo    # (Fixed Public ID) Merchant's main logo
    │       ├── covers/
    │       │   └── cover   # (Fixed Public ID) Merchant's main cover image
    │       ├── items/
    │       │   └── ITEM_ID/  # (Folder) Gallery images for an item are uploaded here
    │       ├── posts/
    │       │   └── POST_ID/  # (Folder) Images for a post are uploaded here
    │       └── stories/      # (Folder) Images for stories are uploaded here
    │
    └── uncategorized/        # Fallback for any unknown asset types
```
...
-   **`items.json`**: व्यापारियों द्वारा बेचे जाने वाले उत्पादों और सेवाओं का विवरण, जिसमें मूल्य निर्धारण, स्टॉक और चित्र शामिल हैं। अब `inventory.isAvailable` फ़ील्ड के साथ उन्नत स्टॉक प्रबंधन सपोर्ट करता है जो दृश्यता (`isActive`) से अलग है।
